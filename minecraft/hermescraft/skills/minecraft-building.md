> Adapted from [`skills/minecraft-building.md`](https://github.com/bigph00t/hermescraft/tree/main/skills) by [bigph00t/hermescraft](https://github.com/bigph00t/hermescraft) (MIT, (c) 2026 bigph00t).
> In this fleet it is installed for: **Steve (foreman) + Reed (builder)** (see `scripts/fleet-model.sh` for brains, `minecraft/minion-controller/config.json` for bodies). `mc` commands are identical; body ports are DuckBot `:3001`, Steve `:3011`, Reed `:3012`, Moss `:3013`, Flint `:3014`, Ember `:3015`.
