...
async function loadHermesCraft() {
  ...
  try {
    ...
    cards.innerHTML = (r.modes || []).map((m) => `<article class="mode-card ${esc(m.status)}">
      ...
      <p>${esc(m.description)}</p><code>${esc(m.upstream_entry)}</code>${r.mode_details?.[m.id] ? (() => { const s = r.mode_details[m.id].deployment_summary; const live = (s.runtime_status || []).filter((x) => x.online).length; return `<div class="mode-deploy-note">${live}/${s.agent_count} configured agent${s.agent_count === 1 ? '' : 's'} live · ${esc(s.activation)}</div>`; })() : ''}
    </article>`).join('');
