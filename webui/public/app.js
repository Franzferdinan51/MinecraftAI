...
async function loadHermesCraft() {
  ...
  try {
    ...
    cards.innerHTML = (r.modes || []).map((m) => `<article class="mode-card ${esc(m.status)}">
      ...
      <p>${esc(m.description)}</p><code>${esc(m.upstream_entry)}</code>${r.mode_details?.[m.id] ? (() => { const s = r.mode_details[m.id].deployment_summary; const live = (s.runtime_status || []).filter((x) => x.online).length; return `<div class="mode-deploy-note">${live}/${s.agent_count} configured agent${s.agent_count === 1 ? '' : 's'} live · ${esc(s.activation)}<details><summary>agent runtime</summary>${s.agents.map((a) => { const rt = (s.runtime_status || []).find((x) => x.name === a.name); return `<div class="mode-agent"><span class="dot ${rt?.online ? 'on' : 'off'}">●</span>${esc(a.name || 'unnamed')} · :${a.port || '?'} · ${rt?.online ? 'online' : 'offline'}</div>`; }).join('')}</details></div>`; })() : ''}
    </article>`).join('');
    ...
    const modeRows = (readiness.operational_modes || []).map((m) => `<div class="readiness-row ${m.status === 'active' ? 'ready' : m.status === 'profile-ready' ? 'blocked' : ''}"><span>${m.status === 'active' ? '✓' : m.status === 'profile-ready' ? '!' : '·'}</span><b>${esc(m.id)}</b><span>${esc(m.status.replace(/-/g, ' '))} · ${m.config_present ? 'config present' : 'no config'} · ${esc(m.launch_policy.replace(/-/g, ' '))}</span></div>`).join('');
    $('#mode-readiness').innerHTML = `<div class="readiness-summary"><b>Active mode:</b> ${esc(readiness.active_mode || 'unknown')}<span>${Object.values(readiness.checks || {}).filter(Boolean).length}/${Object.keys(readiness.checks || {}).length} checks passing</span></div>` + Object.entries(readiness.checks || {}).map(([key, ok]) => `<div class="readiness-row ${ok ? 'ready' : 'blocked'}"><span>${ok ? '✓' : '!'}</span><b>${esc(labels[key] || key)}</b><span>${ok ? 'ready' : 'not ready'}</span></div>`).join('') + `<div class="readiness-subhead">Mode operations</div>${modeRows}`;
